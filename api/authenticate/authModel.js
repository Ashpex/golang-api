...
exports.checkExistUserByEmail = async (email) => {
  try {
    const records = await pool.query('SELECT id FROM "user" WHERE email=$1', [
      email,
    ]);
...
exports.checkExistUserGoogle = async (email) => {
  try {
    const records = await pool.query('SELECT * FROM "user" WHERE email=$1', [
      email,
    ]);
...
exports.updateUserGoogle = async (userObj, lastUserObj) => {
  try {
    let sql = `update "user" set ${
      ...
      !lastUserObj.provider_id_gg
        ? "provider_id_gg='" + userObj.id_provider + "'"
        : ""
